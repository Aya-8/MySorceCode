...
昔の労働ゲーム(煮熟ゲーム)で自分が書いたスクリプトをまとめたフォルダです。
...
昔の労働をテーマに、和紙を作る過程を3つのミニゲームとして表現した体験型ゲームです。木の皮を煮る「煮熟」という作業では、鍋を混ぜる操作をゲームに落とし込み、音声、イラスト、モデル、エフェクトによって直感的に遊べるようにしています。

## 担当範囲

- 煮熟ミニゲーム全体の進行管理
- 右回し、左回し、両方向受付などのフェーズ切り替え
- キーボード・コントローラー入力を回転コマンドとして判定する処理
- 正しい入力・間違った入力に応じた演出、効果音、UI 表示
- 鍋を混ぜる棒の回転制御と、入力方向を示す矢印 UI の実装
- 粉投入、説明、カウントダウン、終了演出などのゲーム内シーケンス制御
